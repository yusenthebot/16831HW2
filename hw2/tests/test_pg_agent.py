...
def test_advantage_standardize_handles_constant_values():
    agent = make_agent(reward_to_go=False, standardize_advantages=True)
    rewards_list = [np.array([1.0]), np.array([1.0])]
    q_values = agent.calculate_q_vals(rewards_list)
    obs = np.zeros((q_values.shape[0], 1), dtype=np.float32)
    terminals = np.array([1, 1], dtype=np.float32)
    advantages = agent.estimate_advantage(obs, rewards_list, q_values, terminals)
    np.testing.assert_allclose(advantages, np.zeros_like(q_values))


def test_baseline_unnormalize_handles_small_variance():
    agent = make_agent(reward_to_go=True, nn_baseline=True, gae_lambda=None)
    q_values = np.array([3.0, 3.0, 3.0])
    rewards_list = [np.zeros_like(q_values)]
    obs = np.zeros((q_values.shape[0], 1), dtype=np.float32)
    terminals = np.array([0, 0, 1], dtype=np.float32)

    agent.actor.run_baseline_prediction = lambda observations: np.zeros_like(q_values)
    advantages = agent.estimate_advantage(obs, rewards_list, q_values, terminals)
    np.testing.assert_allclose(advantages, np.zeros_like(q_values))
