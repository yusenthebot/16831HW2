...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs, rewards_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            q_std = max(np.std(q_values), 1e-8)
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    delta = rewards[i] + self.gamma * values[i + 1] * (1 - is_terminal) - values[i]
                    advantages[i] = (
                        delta
                        + self.gamma * self.gae_lambda * (1 - is_terminal) * advantages[i + 1]
                    )
        ...
        if self.standardize_advantages:
            ...
            adv_std = max(np.std(advantages), 1e-8)
    ...
    def _discounted_return(self, rewards):
        ...
        discounted_returns = np.array([total_return] * len(rewards))
